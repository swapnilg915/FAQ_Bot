...
class ExcelSemanticSimilarity(object):
	...
	def train(self, input_dict):
		...
		sentences = [ self.cleaning_pipeline(sent, input_dict["lang"]) for sent in sentences]
		# sentences = [ word_tokenize(sent) for sent in sentences]

		self.load_word2vec(input_dict["lang"])

		train_time = time.time()
		instance_wmd = WmdSimilarity(sentences, self.model)
		model_path = os.path.join(self.trained_models_dir, input_dict["model_name"] + ".model")
	...
	def test(self, query, lang, model):
		...
		instance_wmd = gensim.similarities.docsim.Similarity.load('en_faqbot_wmd.model')
		training_sentences = json.load(open("training_sentences_en_faqbot.json"))['sentences']
